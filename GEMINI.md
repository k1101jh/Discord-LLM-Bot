...
## 6. 보안 및 환경 변수 관리 규칙
- **민감 정보 하드코딩 절대 금지**: DB 비밀번호, API 키, 인증 토큰 등의 민감한 정보(Credentials)는 코드 내 기본값(Default fallback string 포함)에 직접 작성하거나 노출하지 않습니다.
- **환경 변수 참조**: 모든 접속 URI 및 계정 정보는 `.env` 파일 또는 환경 변수(`os.getenv`)를 통해서만 로드해야 하며, 소스 코드 내 기본값에는 비밀번호가 제거된 형식(`mongodb://localhost:27017/discord_chat_bot`)만 허용합니다.
